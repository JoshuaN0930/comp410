...
def solve(goals, literals):
    #base case goals empty done processing goals list 
    if isinstance(goals, Nil):
        return literals
    #add literal to assigment
    if isinstance(goals.head, Literal):
        value = (goals.head.is_positive == True)
        result = add_literal(literals, goals.head.variable, value)
        if result is None:
            return None
        return solve(goals.tail, result)
    #check if its and op and preform algo for and 
    elif isinstance(goals.head, And):
        return solve(Cons(goals.head.left, Cons(goals.head.right, goals.tail)), literals)
    #check if its or op and split formula into two 
    elif isinstance(goals.head, Or):
        leftSide = solve(Cons(goals.head.left, goals.tail), literals)
        if leftSide is not None:
            return leftSide
        return solve(Cons(goals.head.right, tail), literals)
